...
{
    internal class GetCandlesTinkoff : CandleList
    {
        public GetCandlesTinkoff(string figi, CandleInterval interval, int candlesCount) : base(figi, interval, candles)
        {
            GetCandlesTinkoffAsync(figi, interval, candlesCount);
        }

        internal static async Task<List<CandlePayload>> GetCandlesTinkoffAsync(string figi, CandleInterval candleInterval, int candlesCount)
        {
